...
const DEFAULT_GPS_BAUD_RATE: u32 = 115_200;
...
fn by_id_ara_haric(arananlar: &[&str], haricler: &[&str]) -> Option<String> {
    let klasor = fs::read_dir("/dev/serial/by-id").ok()?;

    for kayit in klasor.flatten() {
        let ad = kayit.file_name();
        let ad = ad.to_string_lossy().to_lowercase();
        let eslesiyor = arananlar
            .iter()
            .all(|aranan| ad.contains(&aranan.to_lowercase()));
        let haric = haricler
            .iter()
            .any(|aranan| ad.contains(&aranan.to_lowercase()));
...
fn baud_oku(env_adi: &str, varsayilan: u32) -> u32 {
    match env::var(env_adi) {
        Ok(deger) => match deger.parse::<u32>() {
            Ok(baud) if baud > 0 => baud,
            _ => {
                eprintln!(
                    "{} geçersiz ({:?}); varsayılan {} baud kullanılacak.",
                    env_adi, deger, varsayilan
                );
                varsayilan
            }
        },
...
fn gps_port_oku() -> String {
    ...
    // Önce doğrudan USB ile bağlanan u-blox/M8N alıcısını ara.
    // USB-TTL dönüştürücü kullanılıyorsa cihaz adı GPS içermeyebilir; bu durumda
    // IDA_GPS_PORT ile /dev/serial/by-id/... yolu açıkça verilmelidir.
    let eski_pico = ["ida_gps_001", "stars-of-hydro", "starsofhydro"];
    by_id_ara_haric(&["u-blox"], &eski_pico)
        .or_else(|| by_id_ara_haric(&["ublox"], &eski_pico))
        .or_else(|| by_id_ara_haric(&["m8n"], &eski_pico))
        .or_else(|| by_id_ara_haric(&["gnss"], &eski_pico))
        .or_else(|| by_id_ara_haric(&["gps"], &eski_pico))
...
fn imu_port_oku() -> String {
    ...
    by_id_ara(&["Embassy", "serial_logger"]).unwrap_or_else(|| DEFAULT_IMU_PORT.to_string())
}
...
pub fn baslat() -> (BeyinKanallari, HaberlesmeGorevleri) {
    ...
    let gps_baud_rate = baud_oku("IDA_GPS_BAUD", DEFAULT_GPS_BAUD_RATE);
    ...
    println!(
        "GPS USB   : {} @ {} baud (NMEA, otomatik baud taramalı)",
        gps_port, gps_baud_rate
    );
