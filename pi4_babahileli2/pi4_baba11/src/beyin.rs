            ...
            if !imu_hazir {
                ...
                match kare_asama {
                    ...
                    KareAsama::SolaDon => {
                        ...
                        let kalan_aci =
                            (KARE_DONUS_ACISI_DEG - kare_donus_birikimli_deg).max(0.0);
                        let donus_tamam = kare_donus_yon_isareti != 0.0
                            && kalan_aci <= KARE_DONUS_TOLERANSI_DEG;

                        if donus_tamam {
                            let bitis_yaw = aci_normalize_360(imu.yaw);
                            println!(
                                "KARE: IMU dönüşü tamamlandı; başlangıç={:.2}°, bitiş={:.2}°, ölçülen={:.2}°. {}. kenar ileri başlıyor.",
                                kare_donus_baslangic_yaw,
                                bitis_yaw,
                                kare_donus_birikimli_deg,
                                kare_kenar_no + 1,
                            ...
                            guncel_mod = AracMod::GorevBekliyor;
                            eprintln!(
                                "KARE İPTAL: IMU ile 90° dönüş {} saniyede tamamlanamadı (ölçülen {:.2}°). Motorlar durduruldu.",
                                KARE_DONUS_MAX_SURE.as_secs(),
                                kare_donus_birikimli_deg,
                            );
                        } else {
                            // Hedefe yaklaşınca daha düşük PWM kullanarak aşmayı azaltır.
                            let donus_pwm = if kalan_aci <= KARE_DONUS_YAVAS_BOLGE_DEG {
