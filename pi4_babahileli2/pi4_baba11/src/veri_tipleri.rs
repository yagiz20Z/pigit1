...
impl Default for MotorEsleme {
    fn default() -> Self {
        // Kullanıcının doğruladığı fiziksel yerleşim:
        // M1 = sol yatay, M2 = sağ yatay, M3 ve M4 = ileri iticiler.
        Self {
            sol: 1,
            ileri1: 3,
            sag: 2,
...
pub struct GidenTelemetri {
    ...
    pub aktif_waypoint: usize,
    pub toplam_waypoint: usize,
    pub hedef_mesafe_m: f32,
    pub gps_hazir: bool,
    pub imu_hazir: bool,
    pub origin_hazir: bool,
    pub telemetri_bagli: bool,
    /// 0=hazır/manuel, 1=rota yok, 2=GPS bekliyor, 3=IMU bekliyor,
    /// 4=origin bekliyor, 5=hedefe dönüyor, 6=ilerliyor,
    /// 7=görev tamamlandı, 8=acil durum.
    pub otonom_durum: u8,
...
impl GidenTelemetri {
    pub fn to_rf_strings(&self) -> (String, String, String) {
        let nav_payload = format!(
            ...
            self.arac_mod as u8,
            self.aktif_waypoint,
            self.toplam_waypoint,
            self.hedef_mesafe_m,
            self.gps_hazir as u8,
            self.imu_hazir as u8,
            self.origin_hazir as u8,
            self.telemetri_bagli as u8,
            self.otonom_durum,
        );
