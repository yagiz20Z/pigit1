...
use crate::veri_tipleri::{DeadReckoningTelemetri, GpsVeri, MotorEsleme, MotorVeri};
...
impl DeadReckoning {
    ...
    pub fn guncelle(
        ...
        if imu_hazir && yaw.is_finite() && dt > 0.0 && esleme.gecerli() {
            let ileri1 = pwm_orani(motor_degeri(motor, esleme.ileri1), self.ayar.pwm_olu_bolge);
            let ileri2 = pwm_orani(motor_degeri(motor, esleme.ileri2), self.ayar.pwm_olu_bolge);
        ...
        let boylam =
            self.baslangic_boylam + self.dogu_m * METRE_BASINA_ENLEM_DERECESI / self.cos_enlem;
