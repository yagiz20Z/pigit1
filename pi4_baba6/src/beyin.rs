...
)
{
    ...
    loop
    {
        ...
        while let Ok(komut) = rx_yki.try_recv()
        {
            match komut
            {
                ...
                GelenTelemetri::ManuelKontrol(ileri, yatay) =>
                {
                    ...
                    son_manuel_komut_zamani = Instant::now();
        ...
        {
            let manuel_komut_guncel = manuel_komut_alindi
                && simdi.duration_since(son_manuel_komut_zamani) <= MANUEL_KOMUT_ZAMAN_ASIMI;

            if manuel_komut_guncel
            {
            ...
            else
            {
                ...
                if manuel_komut_alindi && !manuel_zaman_asimi_bildirildi
                {
                    println!("Manuel UART komutu 500 ms boyunca gelmedi: motorlar durduruldu.");
                    manuel_zaman_asimi_bildirildi = true;
                }
            }
        }
