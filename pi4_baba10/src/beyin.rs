...
const GPS_VERI_ZAMAN_ASIMI: Duration = Duration::from_secs(2);
const IMU_VERI_ZAMAN_ASIMI: Duration = Duration::from_millis(500);
...
fn manuel_motor_karistir(ileri_girdisi: f32, yatay_girdisi: f32, esleme: MotorEsleme) -> MotorVeri {
    // İleri ve yatay bileşen birbirinden bağımsızdır.
    // YKİ joystickleri önce oransal 0.0..1.0 bileşenlere ayırır;
    // burada her bileşen doğrudan 0..1000 PWM komutuna çevrilir.
...
pub async fn nav_task(
    ...
    println!(
        ...
        dr_ayar.max_ileri_hiz_m_s,
        dr_ayar.max_yatay_hiz_m_s,
        dr_ayar.pwm_olu_bolge,
    );
    loop {
        tick.tick().await;
        ...
        // Pico zaman_ms alanı cihaz uptime'ı olabileceği için freshness hesabında kullanılmaz.
        ...
        while let Ok(komut) = rx_yki.try_recv() {
            match komut {
                ...
                GelenTelemetri::DeadReckoningSifirla => {
                    ...
                    println!("DR sıfırlama istendi: güncel GPS konumu ve IMU yaw yeni merkez olacak.");
                }
        ...
        // Tahmini iz ilk geçerli GPS+IMU ile başlar. CMD:DR:RESET geldiğinde
        // o anki GPS konumu ve IMU yaw yeni merkez olur.
...
mod tests {
    ...
    #[test]
    fn ileri_sifirken_sag_motor_oransal_calistirir() {
        let esleme = MotorEsleme::default();
        let motor = manuel_motor_karistir(0.0, 0.35, esleme);
        ...
        assert_eq!(kanal(&motor, esleme.sag), 350);
        assert_eq!(kanal(&motor, esleme.sol), 0);
        assert_eq!(kanal(&motor, esleme.ileri1), 0);
        assert_eq!(kanal(&motor, esleme.ileri2), 0);
    }

    #[test]
    fn ileri_ve_sol_bilesenleri_bagimsiz_pwm_olur() {
        let esleme = MotorEsleme::default();
        let motor = manuel_motor_karistir(0.60, -0.25, esleme);
        let kanal = |m: &MotorVeri, no: u8| match no {
            1 => m.iskeleon,
            2 => m.iskelearka,
            3 => m.sancakon,
        ...
        assert_eq!(kanal(&motor, esleme.ileri1), 600);
        assert_eq!(kanal(&motor, esleme.ileri2), 600);
        assert_eq!(kanal(&motor, esleme.sol), 250);
        assert_eq!(kanal(&motor, esleme.sag), 0);
    }
